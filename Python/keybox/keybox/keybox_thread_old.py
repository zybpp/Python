...
async def copy_files(id,orig_dir_sub):
	sem.acquire()
	sub_file_path = ""
	sub_name = re.sub("-(\d+).", "-" + id + ".", os.listdir(orig_dir)[0])
	...
	if time > 0:
		for i in range(time + 1):
			...
			for id in keybox_list[start:end]:
				copy_tasks.append(asyncio.ensure_future(copy_files(id,orig_dir_sub)))
		...
		for id in keybox_list[start:end]:
			if len(os.listdir(orig_dir)) > 0:
				file_name = re.sub("-(\d+).", "-" + id + ".", os.listdir(orig_dir)[0])
